"""
Basic SWHEar class for continuous monitoring of microphone data.

The Ear class is the basic method for accessing microphone data.
It has additional routines for audiomatically detecting/testing the sound card, channel
and frequency combinations to maximise the probability of finding one that
working in the system (all without user input!).

Although designed as a package, it is simple enough to run
as a completely standalone Python script. Simply drop this .py file into
into your project and import it by filename. Done!
"""
...
class Ear(object):
    ...
    def stream_start(self):
        """Adds data to self.data until termination signal occurs."""
        self.initiate()
    ...
    def softEdges(self, data, fracEdge=0.05):
    ...
    def getFiltered(self, freqHighCutoff=50):
        """Filters the signal using lowpass filter."""
        if freqHighCutoff <= 0:
            return self.data
